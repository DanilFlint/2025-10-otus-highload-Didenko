...
`docker exec -it highload-cache-container redis-cli --raw` - войти в контейнер redis
`keys *` or `scan 0` - посмотреть все ключи
`set <key> <value>`
...
`LPOP key` — удаляет и возвращает первый (левый) элемент.  
`RPOP key` — удаляет и возвращает последний (правый) элемент.  
Диапазоны:  
`LRANGE key start stop` — возвращает элементы из указанного диапазона (например,LRANGE mylist 0 - 1 вернет весь список).  
Информация:  
`LLEN key` — возвращает длину списка.    
`LINDEX key index` — получает элемент по индексу.   

Основные десериализаторы в Spring Data Redis:
StringRedisSerializer: Самый популярный вариант для ключей и строковых значений. Обеспечивает прямую запись строк.
GenericJackson2JsonRedisSerializer: Лучший выбор для сложных Java-объектов. Преобразует объекты в JSON, сохраняя структуру, что делает данные читаемыми вне Java-приложений.
Jackson2JsonRedisSerializer<T: Аналогичен предыдущему, но требует указания конкретного класса при инициализации.
JdkSerializationRedisSerializer: По умолчанию использует нативную сериализацию Java. Не рекомендуется из-за уязвимостей безопасности и нечитаемости данных. 